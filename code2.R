...
corrplot(cor(Data.matrix.base))
model.outcome = list()
X<- data.matrix(Data.matrix.base[c(2:16)])
...
#prediction
m.forward.test.pred <- predict(forward_train, newdata = test.set)
#calculate RMSE for the test data set
print(paste("RMSE on test set:", rmse2(y.test,m.forward.test.pred)))
# store model object and results of rmse in list object name mode.outcom
model.outcome[['forward_model']] = list('model' = forward_train, 
                                     'rmse' =  data.frame('name'= 'Forward model',
                                                          'train.RMSE' = rmse(forward_train),
                                                          'test.RMSE' = rmse2(y.test,m.forward.test.pred)))

summary(forward_train)

test.model <- lm(formula = y.test ~1, data = test.set)
test.scope <- as.formula(lm(y.test ~., data = test.set))
forward_test <- step(test.model, scope = test.scope, direction = "forward", trace = 0)
...
#define (empty) model
backward_model <- lm(formula = y.train ~., data = train.set)
# call the step function 
backward_Elimi <- step(backward_model, direction = "backward", trace = 0)
backward_Elimi 
#calculating rmse on training set
print(paste("RMSE on training set :", rmse(backward_Elimi)))
#prediction
m.backward.test.pred <- predict(backward_Elimi, newdata = test.set)
#calculate RMSE for the test data set
print(paste("RMSE on test set:", rmse2(y.test,m.backward.test.pred)))
# store model object and results of rmse in list object name mode.outcom
model.outcome[['Backward model']] = list('model' = forward_train, 
                                        'rmse' =  data.frame('name'= 'Backward model',
                                                             'train.RMSE' = rmse(backward_Elimi),
                                                             'test.RMSE' = rmse2(y.test,m.backward.test.pred)))


summary(backward_Elimi)

backward_model_test <- lm(formula = y.test ~., data = test.set)
# call the step function 
backward_Elimi_test <- step(backward_model_test, direction = "backward", trace = 0)

summary(backward_Elimi_test) 
